const hexagrams = [
    ...
    },
    {
        ...
        }, 
        {
          ...
          },
          {
            ...
            "chinese_name": "(Xiǎo Guò)",
            "representation": "001100",
 ...
 export function getCoinFlipLine() {
      let sum = 0;
      for (let i = 0; i < 3; i++) {
  ...
  export function mapLinesToHexagramDetails(lines: LineType[]) {
      ...
      const hexagram = hexagrams.find((hex: any) => hex.representation === representation);
      ...
      return {
          ...
          changing: hexagram.lines.filter((line, index) => {if(lines[index].changingTo !== null) {return line}})
      };
  }
  ...
  function findMissingHexagrams() {
    const allPossibleRepresentations = new Set<string>();
  
    // Generate all 64 possible 6-bit binary strings
    for (let i = 0; i < 64; i++) {
      const binaryString = i.toString(2).padStart(6, '0');
      allPossibleRepresentations.add(binaryString);
    }
  
    // Remove representations present in hexagrams array
    for (const hex of hexagrams) {
      allPossibleRepresentations.delete(hex.representation);
    }
  
    return Array.from(allPossibleRepresentations);
  }
  
  function findDuplicatedRepresentations(hexagrams: any[]) {
    const representationCount: { [key: string]: number } = {};
    const duplicates: string[] = [];
  
    for (const hexagram of hexagrams) {
      if (representationCount[hexagram.representation]) {
        representationCount[hexagram.representation]++;
      } else {
        representationCount[hexagram.representation] = 1;
      }
    }
  
    for (const [representation, count] of Object.entries(representationCount)) {
      if (count > 1) {
        duplicates.push(representation);
      }
    }
  ...
  export function generateReading() {
    return mapLinesToHexagramDetails(generateHexagram());
  }
